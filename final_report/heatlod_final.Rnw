...
Correlation describes the strength and direction of a linear association. The height of a ball thrown up in the air follows a quadratic pattern with time, therefore, in this example, there is a relationship between height of the ball and time. The relationship, however, is not linear, and so would not be described well with a measure of correlation. It is known that gravity {\it causes} the ball fall. Correlation, however, can only tell about associations, not causal relationships. An example of association might be that more students go to the library in the beginning of December than the end. There is an association between the number of students in the library and the time in December, but it is not December that is causing more students to go to the library, it is finals week. There is an association between the day in December and the number of students in the library, but the day in December does not {\it make} students go to the library at a certain rate.
...
mergedbzn<-read.csv(file.choose(),header=T)
summary(mergedbzn)
names(mergedbzn)
...
bozeman16<-read.csv(file.choose(),header=T)
summary(bozeman16)
...
##Load Data (Bozeman 2014)
bozeman14<-read.csv(file.choose(),header=T)
summary(bozeman14)
names(bozeman14)
...
bozeman15<-read.csv(file.choose(),header=T)
summary(bozeman15)
names(bozeman15)
...
moccasin16<-read.csv(file.choose(),header=T)
summary(moccasin16)
...
moccasin15<-read.csv(file.choose(),header=T)
summary(moccasin15)
names(moccasin15)
...
mergedmc<-read.csv(file.choose(),header=T)
summary(mergedmc)
names(mergedmc)
