...
We include 95 percent confidence intervals for the true mean percent lodging at each site-year combination. In order to get a better sense for which groups differ, we performed Tukey-Kramer pairwise comparisons. The results of that analysis are included below. It appears that the 
...
pander(confint(lm.cellmeans)) #gives a better confidence interval
@
